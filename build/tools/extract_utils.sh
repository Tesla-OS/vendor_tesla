...
function setup_vendor() {
    ...
    export PRODUCTMK="$TESLA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$TESLA_ROOT"/vendor/tesla/build/tools/smali/baksmali.jar
        export SMALIJAR="$TESLA_ROOT"/vendor/tesla/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$TESLA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$TESLA_TARGET" >/dev/null; then
...
function extract() {
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$TESLA_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$TESLA_ROOT"/vendor/tesla/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$TESLA_ROOT"/"$OUTDIR"/radio
